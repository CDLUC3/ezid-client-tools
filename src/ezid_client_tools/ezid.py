...
# in %-encoded form.
#
# Usage: ezid [options] credentials operation...
#
#   options:
...
OPERATIONS = {
    ...
    "mint": (lambda x: x % 2 == 1, False),
    "create": (lambda x: x % 2 == 1, True),
    "view": (1, True),
    "update": (lambda x: x % 2 == 1, False),
    "delete": (1, False),
    "login": (0, False),
    "logout": (0, False),
    "status": (0, False),
...
USAGE_TEXT = """Usage: ezid [options] credentials operation...
...
def main():  # noqa: C901
    global _options, _server, _opener, _cookie
    ...
    if (
        ...
        isinstance(types.LambdaType, OPERATIONS[operation][0])
